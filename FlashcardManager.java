...
public class FlashcardManager {
    private static ArrayList<Card> cardList = new ArrayList<Card>(); // Need an ArrayList reference for Android intent passing.
    ...
    public static ArrayList<Card> readQuestions(InputStream is) {
    	...
    	Scanner inFile = null;
    	String [] cardArray;
    	try { inFile = new Scanner(new File("is"));
    	while(inFile.hasNext()){
    		s = inFile.next();
    		cardArray = s.split(";");
    		Card card = new Card(s, null, 0, s);
    ...
    public static ArrayList<Card> readQuestions(String url)  {
    	...
    	Answer answer = null;
    	try{ 
    		URL remoteConnection = new URL(url);
		URLConnection connection = remoteConnection.openConnection();
		remoteIn = new Scanner(connection.getInputStream());
		while (remoteIn.hasNext()){
			inline = remoteIn.next();
			cardArray = inline.split(";");
			String x = cardArray[3];
			
			if (x.equals("TF")) {
				answer = new TruthAnswer(Boolean.valueOf(cardArray[4]));
			} else if (x.equals("MC")){
				possibleAnswer = new String [5];
				
				answer = new MultipleChoiceAnswer(List.valueOf(cardArray[4], ));
			} else if (x.equals("SA")){
				answer = new ShortAnswer(cardArray[4], Boolean.valueOf(cardArray[5]));
			} 
			
			Card card = new Card(cardArray[0], answer, Integer.getInteger(cardArray[1]), cardArray[4]);
			cardList.add(card);
		}	
